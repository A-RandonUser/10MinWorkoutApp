...
import com.example.workoutinminutes.databinding.ItemExerciseStatusBinding
...
class ExerciseStatusAdapter(val items: ArrayList<ExerciseModel>):
    RecyclerView.Adapter<ExerciseStatusAdapter.ViewHolder>(){
        ...
        class ViewHolder(binding: ItemExerciseStatusBinding):
            ...
            //the tvitem is the individual item we have created in item exercise status xml
                val tvItem = binding.tvItem
            }

    //to inflate outside of activity we need to use the from parent.context
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        return ViewHolder(ItemExerciseStatusBinding.inflate(LayoutInflater.from(parent.context),parent,false))
    }
    //we will call this method for each position so 10 times
